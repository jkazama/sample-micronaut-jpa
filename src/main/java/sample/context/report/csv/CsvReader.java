...
public class CsvReader {
    ...
    private String unescape(String input) {
        if (StringUtils.isEmpty(input) || layout.isNonQuote()) {
            return input;
        }
        ...
        for (String eol : eols) {
            if (quoteless.contains(eol)) {
                eolsAny = true;
                break;
            }
        }
        if (quoteless.contains(String.valueOf(delim)) || quoteless.contains(String.valueOf(quote)) || eolsAny) {
            unescape = quoteless.replaceAll(quoteStr + quoteStr, quoteStr);
        } else {
            unescape = input;
        }
